...
class LitertLmFileBuilder:
  ...
  def _prepare_tokenizer_metadata(
    ...
    if (
      ...
      assert (
          None not in self._tokenizers_by_model_type
          and TfLiteModelType.PREFILL_DECODE.value
          not in self._tokenizers_by_model_type
      ), "Tokenizer already added."
    else:
      assert (
          model_type_key not in self._tokenizers_by_model_type
  ...
  def add_sentencepiece_tokenizer(
    ...
    Args:
      ...
      model_type: The model type this tokenizer corresponds to (e.g.
        TfLiteModelType.PREFILL_DECODE or TfLiteModelType.NAM_TEXT_ENCODER).
