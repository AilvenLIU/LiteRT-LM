...
"""Inspect subcommand for LiteRT-LM CLI."""
...
@click.command(cls=help_formatter.ColorCommand, name="inspect")
...
def inspect_model(
  ...
  """Inspects a LiteRT-LM model and prints its capabilities.
  ...
  Example usage:
    ...
    litert-lm inspect my-model-id

    litert-lm inspect /path/to/model.litertlm

    # Inspect directly from a HuggingFace repository
    litert-lm inspect --from-huggingface-repo org/repo model.litertlm
  """
  ...
  click.echo(f"  Sampler Type:           {sampler_params.type.name}")
  click.echo(f"  Sampler Temp:           {sampler_params.temperature:.2f}")
  click.echo(f"  Sampler Top K:          {sampler_params.top_k}")
  click.echo(f"  Sampler Top P:          {sampler_params.top_p:.2f}")
