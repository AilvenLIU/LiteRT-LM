...
"""Unit tests for the LiteRT-LM inspect command."""
...
import litert_lm_cli.commands.inspect as inspect_cmd
...
class InspectTest(absltest.TestCase):
  ...
  def test_inspect_success(self):
    ...
    self.mock_capabilities.default_sampler_params = litert_lm.SamplerParameters(
        type=litert_lm.SamplerType.TOP_P,
        temperature=0.7,
        top_k=40,
        top_p=0.9,
    )
    ...
    self.assertIn("LiteRT-LM Model Inspection Report", result.output)
    ...
    self.assertIn("Sampler Type:           TOP_P", result.output)
  ...
  def test_inspect_load_error(self):
  ...
  def test_inspect_from_huggingface(self):
    mock_resolve = self.enter_context(
        mock.patch.object(
            inspect_cmd.cli_helpers,
            "resolve_model_file",
    ...
    mock_download = self.enter_context(
        mock.patch.object(
            inspect_cmd.huggingface_download,
    ...
    self.mock_capabilities.default_sampler_params = litert_lm.SamplerParameters(
        type=litert_lm.SamplerType.UNSPECIFIED,
        temperature=0.0,
        top_k=0,
        top_p=0.0,
    )
  ...
  def test_inspect_no_args_prompts_imported(self):
    mock_resolve = self.enter_context(
        mock.patch.object(
            inspect_cmd.cli_helpers,
            "resolve_model_file",
    ...
    runner = CliRunner()
    result = runner.invoke(inspect_cmd.inspect_model)
