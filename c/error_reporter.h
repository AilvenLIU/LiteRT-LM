...
// and remains valid until the next LiteRT LM C API error on the same thread
// or until `litert_lm_clear_last_error` is called.
//
// Following standard C API conventions, error state is set only upon failure.
// Functions that succeed do not clear or modify the error state. Callers should
// check the return value of an API function to determine whether an error
// occurred before inspecting the error message or code.
//
...
//
// Following standard C API conventions, error state is set only upon failure.
// Functions that succeed do not clear or modify the error state.
//
