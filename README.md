...
on-device inference with MTP, delivering >2x faster decode speeds on mobile GPUs
...
👉 Try on Linux, macOS, Windows or Raspberry Pi with the
...
-   **v0.11.0**: Support Multi-token Prediction (MTP) for Gemma 4. Expand LiteRT-LM CLI to run natively on Windows with CPU and GPU backends.
