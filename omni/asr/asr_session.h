...
class AsrSession {
 public:
  ...
  absl::Status ProcessNextChunk(TextMerger::MergeResult* result);

  // Flushes remaining unconfirmed text at stream end.
  absl::Status Flush(TextMerger::MergeResult* result);
 ...
 private:
  ...
  // Pre-allocated reusable scratch buffers to eliminate per-chunk allocations
  std::vector<float> pcm_buffer_;
  std::vector<float> mel_buffer_;
  std::vector<SpeechDecoder::DecodedToken> token_buffer_;
  std::vector<Detokenizer::Word> word_buffer_;
};
