...
class SortedStringIndex {
 public:
  // Constructs an empty index (`size() == 0`), allowing a caller to hold a
  // default-initialized member before a lexicon blob is loaded.
  SortedStringIndex();
...
class StringValueTable {
 public:
  // Constructs an empty value table (`size() == 0`).
  StringValueTable();
