...
// Parses an offset array of `size + 1` words, checking that it is ascending and
// stays inside `body`. Returns the entry count.
...
SortedStringIndex::SortedStringIndex() = default;
...
absl::StatusOr<SortedStringIndex> SortedStringIndex::Create(
  ...
  LITERT_ASSIGN_OR_RETURN(const int size,
...
StringValueTable::StringValueTable() = default;
