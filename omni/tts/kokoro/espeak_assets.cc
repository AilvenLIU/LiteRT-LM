...
constexpr char kEspeakPhonemeTableFile[] = "phontab";
...
absl::StatusOr<std::string> UnpackEspeakDataFromLitertLm(
  ...
  if (std::filesystem::exists(dest_dir / kEspeakPhonemeTableFile, ec)) {
  ...
  absl::Status status = WriteZipEntries(archive, staging_dir);
  if (!status.ok()) {
    std::filesystem::remove_all(staging_dir, ec);
    return status;
  }
  if (!std::filesystem::exists(staging_dir / kEspeakPhonemeTableFile, ec)) {
    std::filesystem::remove_all(staging_dir, ec);
    return absl::InvalidArgumentError(absl::StrCat(
