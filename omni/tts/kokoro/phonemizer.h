...
// the token stream, and the `switched` flag lets callers log the offending
// word instead of failing silently.
//
// args
// - phonemes: Raw phoneme string as returned by espeak-ng.
// - switched: If non-null, set to true when at least one escape was removed.
//
// returns
...
// LITERT_ASSIGN_OR_RETURN(std::vector<int> ids,
