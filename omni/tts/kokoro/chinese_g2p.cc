...
absl::StatusOr<std::unique_ptr<cppjieba::DictTrie>> CreateDictTrie(
    const CjkBlob& blob) {
  LITERT_ASSIGN_OR_RETURN(const absl::string_view w_idx, blob.Section("w-idx"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view w_key, blob.Section("w-key"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view w_frq, blob.Section("w-frq"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view w_tot, blob.Section("w-tot"));
  LITERT_ASSIGN_OR_RETURN(const SortedStringIndex words,
...
absl::StatusOr<std::string> UnpackHmmModelToCacheDir(
  ...
  LITERT_ASSIGN_OR_RETURN(const absl::string_view hmm_st,
                          blob.Section("hmm-st"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view hmm_tr,
                          blob.Section("hmm-tr"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view hmm_en,
                          blob.Section("hmm-en"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view hmm_ec,
                          blob.Section("hmm-ec"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view hmm_ep,
                          blob.Section("hmm-ep"));
  ...
  if (!cache_dir.empty() && std::filesystem::exists(dest_path, ec) &&
  ...
  std::filesystem::path staging_path =
      base_dir /
      absl::StrCat("kokoro_zh_hmm_model.utf8.unpacking.", unique_id);
  {
...
absl::StatusOr<ChineseG2p> ChineseG2p::Create(const CjkBlob& blob,
  ...
  LITERT_ASSIGN_OR_RETURN(std::unique_ptr<cppjieba::DictTrie> dict_trie,
                          CreateDictTrie(blob));
  LITERT_ASSIGN_OR_RETURN(const std::string hmm_model_path,
                          UnpackHmmModelToCacheDir(blob, cache_dir));
  auto hmm_model = std::make_unique<cppjieba::HMMModel>(hmm_model_path);
  auto segmenter = std::make_unique<cppjieba::MixSegment>(dict_trie.get(),
                                                          hmm_model.get());
  segmenter->ResetSeparators("");

  LITERT_ASSIGN_OR_RETURN(const absl::string_view p_idx, blob.Section("p-idx"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view p_key, blob.Section("p-key"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view p_vidx,
                          blob.Section("p-vidx"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view p_val, blob.Section("p-val"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view c_idx, blob.Section("c-idx"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view c_key, blob.Section("c-key"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view c_vidx,
                          blob.Section("c-vidx"));
  LITERT_ASSIGN_OR_RETURN(const absl::string_view c_val, blob.Section("c-val"));

  LITERT_ASSIGN_OR_RETURN(const SortedStringIndex phrases,
                          SortedStringIndex::Create(p_idx, p_key));
  LITERT_ASSIGN_OR_RETURN(const StringValueTable phrase_values,
                          StringValueTable::Create(p_vidx, p_val));
  if (phrases.size() != phrase_values.size()) {
    return absl::InvalidArgumentError(
        absl::StrCat("zh-lexicon phrase key count (", phrases.size(),
                     ") does not match value count (", phrase_values.size(),
                     ")"));
  }

  LITERT_ASSIGN_OR_RETURN(const SortedStringIndex characters,
                          SortedStringIndex::Create(c_idx, c_key));
  LITERT_ASSIGN_OR_RETURN(const StringValueTable character_values,
...
void ChineseG2p::MmsegCut(absl::string_view word,
  ...
  while (!remain.empty()) {
    ...
    if (last_valid_bytes > 0) {
      ...
      int prev = 0;
      for (int end_bytes : char_ends) {
        pieces->push_back(remain.substr(prev, end_bytes - prev));
        prev = end_bytes;
      }
      return;
    }
...
absl::StatusOr<std::string> ChineseG2p::TextToIpa(
    absl::string_view text) const {
  LITERT_ASSIGN_OR_RETURN(const std::vector<absl::string_view> words,
                          Segment(text));
