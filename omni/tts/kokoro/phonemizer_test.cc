...
std::string BuildMinimalChineseBlobForPhonemizerTest() {
  ...
  return BuildCjkBlob({
      {"w-idx", w_idx}, {"w-key", w_key}, {"w-frq", w_frq}, {"w-tot", w_tot},
      {"hmm-st", hmm_st}, {"hmm-tr", hmm_tr}, {"hmm-en", hmm_en},
      {"hmm-ec", ""}, {"hmm-ep", ""},
      {"p-idx", p_idx}, {"p-key", p_key}, {"p-vidx", p_vidx}, {"p-val", p_val},
      {"c-idx", c_idx}, {"c-key", c_key}, {"c-vidx", c_vidx}, {"c-val", c_val},
  });
}
...
TEST(PhonemizerTest, ChineseTextNormalizationMatchesTheWrittenOutForm) {
  ...
  ASSERT_OK_AND_ASSIGN(
      auto phonemizer,
      KokoroPhonemizer::Create(GetTestEspeakDataDir(), "cmn", {}, rules,
                               zh_blob));
...
TEST(PhonemizerTest, StripLanguageSwitches) {
  bool switched = false;
  EXPECT_EQ(StripLanguageSwitches("ni↓xau↓", &switched), "ni↓xau↓");
  EXPECT_FALSE(switched);
  EXPECT_EQ(StripLanguageSwitches("(en)tʃˈaɪniːz(ja)lˈe̞tə", &switched),
            "tʃˈaɪniːzlˈe̞tə");
  EXPECT_TRUE(switched);
  EXPECT_EQ(StripLanguageSwitches("a(pt-br)b", &switched), "ab");
  EXPECT_TRUE(switched);
  EXPECT_EQ(StripLanguageSwitches("(hello)", &switched), "(hello)");
  EXPECT_FALSE(switched);
}
