...
TEST(EngineSettingsTest, MaxVisionTokenPerImage) {
  auto model_assets = ModelAssets::Create("test_model_path_1");
  ASSERT_OK(model_assets);
  auto settings = EngineSettings::CreateDefault(*model_assets);
  ASSERT_OK(settings);

  EXPECT_FALSE(settings->GetMaxVisionTokenPerImage().has_value());

  settings->SetMaxVisionTokenPerImage(200);
  EXPECT_TRUE(settings->GetMaxVisionTokenPerImage().has_value());
  EXPECT_EQ(settings->GetMaxVisionTokenPerImage().value(), 200);
}
