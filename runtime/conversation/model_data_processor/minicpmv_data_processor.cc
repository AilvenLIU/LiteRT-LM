...
// Returns the bytes of the first image content item found in `messages`,
// reading from "path", base64 "blob", or taking inline "bytes".
absl::StatusOr<std::string> ExtractFirstImageBytes(
  ...
  for (const auto& message : messages) {
    ...
    for (const auto& item : message["content"]) {
      ...
      if (item.contains("bytes")) {
        return item["bytes"].get<std::string>();
      }
      ...
      return std::string(static_cast<const char*>(mmap_file->data()),
                         mmap_file->length());
    }
  }
...
MiniCpmVDataProcessor::ToInputDataVectorImpl(
  ...
  // The chat template renders each image as the literal "<image_soft_token>".
  // We replace that marker with the official multi-slice layout
  ...
  ABSL_ASSIGN_OR_RETURN(const std::string image_bytes,
                        ExtractFirstImageBytes(messages));

  // Slice, then pad each strip to the fused signature length. The map-based
  // Encode token count is ceil(L / shrink) with
  // shrink = L / kMiniCpmVTokensPerSlice (= 17), so L = 1088 always yields 64
  // soft tokens.
  MiniCpmVSliceConfig slice_config;
  ABSL_ASSIGN_OR_RETURN(MiniCpmVSliced sliced,
                        PreprocessImageSliced(image_bytes, slice_config));
  ...
  // Split the rendered prompt at the single soft-token marker.
  const size_t marker_pos = rendered_template_prompt.find(kImageSoftToken);
  const std::string prefix =
      (marker_pos == std::string::npos)
          ? rendered_template_prompt
          : rendered_template_prompt.substr(0, marker_pos);
  const std::string suffix =
      (marker_pos == std::string::npos)
          ? ""
          : rendered_template_prompt.substr(marker_pos +
                                            kImageSoftToken.size());

  // Build the interleaved InputData.
  std::vector<InputData> output;

  AppendText(prefix, output);
  // Image id and thumbnail.
  AppendText("<image_id>0</image_id><image>", output);
