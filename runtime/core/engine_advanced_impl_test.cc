...
TEST(EngineTest, CreateEngine_MaxVisionTokenPerImage_NoVisionSettingsFails) {
  auto task_path =
      std::filesystem::path(::testing::SrcDir()) /
      "litert_lm/runtime/testdata/test_lm_new_metadata.task";
  auto model_assets = ModelAssets::Create(task_path.string());
  ...
  engine_settings->GetMutableMainExecutorSettings().SetMaxNumTokens(
      kMaxNumTokens);
  engine_settings->GetMutableMainExecutorSettings().SetCacheDir(":nocache");
  engine_settings->SetMaxVisionTokenPerImage(200);
...
TEST(EngineTest, CreateEngine_MaxVisionTokenPerImage_InvalidValueFails) {
  auto task_path =
      std::filesystem::path(::testing::SrcDir()) /
      "litert_lm/runtime/testdata/test_lm_new_metadata.task";
  auto model_assets = ModelAssets::Create(task_path.string());
  ...
  engine_settings->GetMutableMainExecutorSettings().SetMaxNumTokens(
      kMaxNumTokens);
  engine_settings->GetMutableMainExecutorSettings().SetCacheDir(":nocache");
  engine_settings->SetMaxVisionTokenPerImage(-5);

  EXPECT_THAT(
      CreateEngine(*engine_settings),
      testing::status::StatusIs(
          absl::StatusCode::kInvalidArgument,
          testing::HasSubstr("max_vision_token_per_image must be positive")));
}

TEST(EngineTest,
     CreateEngine_VisionExecutorSettings_WithoutMaxVisionTokenPerImage) {
  auto task_path =
      std::filesystem::path(::testing::SrcDir()) /
      "litert_lm/runtime/testdata/test_lm_new_metadata.task";
  auto model_assets = ModelAssets::Create(task_path.string());
