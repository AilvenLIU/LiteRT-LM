...
// Auto-selects the text encoder signatures to load when either input length
// bound is set, records them in the executor settings, and returns what was
// selected. Returns nullopt when neither bound is set, which leaves every
// signature in the model loaded.
//
// Assumes the bounds have already been validated by
