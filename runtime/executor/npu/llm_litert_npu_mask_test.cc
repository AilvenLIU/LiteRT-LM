...
// =============================================================================
// Regression tests for two defects fixed alongside these tests. Both failed
// against the previous implementation; they are kept here so the two failure
// modes stay pinned.
// =============================================================================
...
// UpdateInterleavedSWAMasks even when only the global mask is bound. That used
// to forward `seq_q_local`, which is only assigned inside
// `if (mask_local_buf)` and so was still 0: every fill loop was bounded by
// seq_q == 0, nothing was written, and OkStatus was returned over whatever the
// buffer previously held. HWMaskUpdate now derives one query length from
// whichever mask is bound. This test seeds the buffer with a sentinel so a
...
// guards with `if (k >= seq_k) break;`. That was safe while capacity was
// derived as `seq_k_local - seq_q`, but capacity now arrives from
// NpuModelGeometry, and UpdateInterleavedSWAMasks only substitutes
// `seq_k - seq_q` when the supplied capacity is <= 0. Because
// geometry.global_cache_length is a token budget rather than a tensor
// dimension, it can legitimately exceed the mask width, so
// UpdateInterleavedSWAMasks now rejects a capacity that does not fit the bound
