...
absl::Status UpdateInterleavedSWAMasks(
  ...
  if (local_ptr != nullptr) {
    RET_CHECK(local_capacity + seq_q <= seq_k_local)
        << "Local mask capacity (" << local_capacity << ") plus query length ("
        << seq_q << ") exceeds the local mask key dimension (" << seq_k_local
        << ").";
  }
  if (global_ptr != nullptr) {
    RET_CHECK(global_capacity + seq_q <= seq_k_global)
        << "Global mask capacity (" << global_capacity
        << ") plus query length (" << seq_q
        << ") exceeds the global mask key dimension (" << seq_k_global << ").";
  }
