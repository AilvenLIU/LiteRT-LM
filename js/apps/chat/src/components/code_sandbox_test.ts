...
describe('CodeSandbox', () => {
  ...
  let setSandboxIframeHtmlSpy: jasmine.Spy;
  ...
  beforeEach(() => {
    ...
    setSandboxIframeHtmlSpy = spyOn(util, 'setSandboxIframeHtml').and.callFake((iframe, html) => {
      activeIframe = iframe;
  ...
  it('runs normally after explicit termination', async () => {
    const promise1 = sandbox.run('15 + 15');
    await new Promise(resolve => setTimeout(resolve, 0));
