...
litert::lm::SuppressTokensConfig CreateSuppressTokensConfigFromJni(
  ...
  jmethodID get_suppress_tokens_mid =
      env->GetMethodID(cls, "getSuppressTokens", "()Ljava/util/List;");
  jobject list_obj = env->CallObjectMethod(suppress_tokens_config_obj,
                                           get_suppress_tokens_mid);
  ...
  if (list_obj != nullptr) {
    ...
    jmethodID get_mid =
        env->GetMethodID(list_cls, "get", "(I)Ljava/lang/Object;");

    jint size = env->CallIntMethod(list_obj, size_mid);
    jclass integer_cls = env->FindClass("java/lang/Integer");
    jmethodID int_value_mid = env->GetMethodID(integer_cls, "intValue", "()I");

    for (jint i = 0; i < size; ++i) {
      jobject item = env->CallObjectMethod(list_obj, get_mid, i);
      if (item != nullptr) {
        jint val = env->CallIntMethod(item, int_value_mid);
