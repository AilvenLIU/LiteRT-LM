...
LITERTLM_JNIEXPORT jint JNICALL JNI_METHOD(nativeConversationGetTokensCount)(
    JNIEnv* env, jclass thiz, jlong conversation_pointer) {
  Conversation* conversation =
      reinterpret_cast<Conversation*>(conversation_pointer);

  auto tokens_count = conversation->GetTokensCount();
  if (!tokens_count.ok()) {
    ThrowLiteRtLmJniException(
        env, "Failed to get tokens count: " + tokens_count.status().ToString());
    return 0;
  }
